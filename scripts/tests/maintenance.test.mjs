...
test('vue2 learning pages do not contain draft-note wording', () => {
  ...
  const bannedPatterns = [/说的那么高端/, /类似于java中的jar包/i, /1111|2222/, /原文件为空壳/];
  const failures = [];
...
test('gen-vitepress-sidebar --check validates generated docs nav without rewriting', () => {
  const generatedFiles = {
    sidebar: readFileSync(join(ROOT, 'docs/.vitepress/sidebar.generated.mts'), 'utf8'),
  ...
  try {
    ...
    writeFileSync(join(ROOT, 'docs/.vitepress/sidebar.generated.mts'), generatedFiles.sidebar);
