...
Vue 2.7 时代的 demo（`libs/vue.js` 为 **2.7.16**，Vue 2 最终版）。`apps/vue2/src/` 下共 **57** 个 `.html` 文件，每个文件可直接在浏览器打开，无需构建。
...
`libs/vue.js` `libs/vue-router.js` `libs/vuex.js` `libs/es6-promise.auto.js`（版本见 [`libs/README.md`](libs/README.md)）。通用库 `axios` / `lodash` / `js-cookie` 见 [`../../packages/shared/libs/README.md`](../../packages/shared/libs/README.md)。
...
| `03-生命周期/01-八个钩子速记.html` | Vue 2 八个生命周期钩子速记 |
