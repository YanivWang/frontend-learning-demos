...
function addDemoPageClass(content) {
  ...
  return content.replace(/<body(\s[^>]*)?>/i, (full, attrs = '') => {
    if (/class\s*=/.test(attrs)) {
      return full.replace(/class\s*=\s*["']([^"']*)["']/i, 'class="$1 demo-page"');
    }
...
function migrateNavBlock(navRaw) {
  ...
  const links = [...inner.matchAll(/<a\s[\s\S]*?<\/a>/gi)].map((m) => m[0].trim());
...
function buildNotesBlock(h1, hint, notesInner, theme, rel, content) {
  ...
  const hintText = hint?.replace(/<\/?p[^>]*>/gi, '').trim() || defaultHint(rel, content);
  const hintHtml = hint && hint.startsWith('<p') ? hint : `<p class="hint">${hintText}</p>`;
...
async function main() {
  ...
  console.log(`[migrate-demo-shell] 完成：更新 ${updated}，跳过 ${skipped}，共 ${files.length}`);
  if (issues.length) {
    console.warn('[migrate-demo-shell] 未迁移：\n' + issues.map((i) => `  • ${i}`).join('\n'));
  }
}
