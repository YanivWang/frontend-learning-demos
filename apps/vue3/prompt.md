# 任务：批量升级 `apps/vue3/` 下所有 Vue 3 Demo 页

你是本仓库的前端学习 Demo 维护者。请按**与标杆一致的质量标准**，对 `apps/vue3/` 目录下**全部 `.html` demo 页**（共 41 个）进行内容修复、补全与增强。
...
**与 AI 沟通：** 新对话中 `@apps/vue3/prompt.md` `@CONVENTIONS.md` 及上述标杆文件，再粘贴 §十一「开始执行」或 §十「续跑指令」。
...
1. **扫描清单**：按本文 §十二，或 `find apps/vue3 -name '*.html' | sort`，从 `01-基础语法` → `08-原理与性能面试` 顺序处理。
2. **逐个文件**：遵循 §零 工作流；自检 **头注释 ↔ NOTES ↔ PAGE_DOM ↔ SCRIPT 四者一致**。
3. **同主题去重**：相邻 demo 分工明确——基础页讲语法 + 可运行交互，进阶页讲边界 + 面试题；避免两篇重复同一演示。
4. **本任务默认一次改完**：共 41 个文件；若上下文不足，每轮至少 **8～10 个**文件，未完成用 §十 续跑。
5. **全部完成后**：
   - 运行 `npm run validate`
   - 抽样打开 4～6 个改过的页面，确认 Console 无报错、交互符合 NOTES
...
- **已改数 / 剩余数**
- **已完成标杆（跳过或仅一致性检查）**：`01-基础语法/01-应用实例与模板语法.html`
- **未改动文件及原因**
- **需人工工程化环境验证的文件**（如 Vitest CI、完整 Router history 模式）
...
| `07-路由状态工程化/` | Router 4、Pinia、Vite、script setup 宏、可运行示例、Vitest 面试边界 |
| `08-原理与性能面试/` | Proxy 响应式、虚拟 DOM/diff/key、调度器/nextTick、Vue2 vs Vue3、性能优化 |
...
### `apps/vue3/src/01-基础语法/01-应用实例与模板语法.html`（**已完成**）
...
按 prompt.md 批量升级 apps/vue3/ 下全部 demo（共 41 个 html）。
跳过已完成的 01-应用实例与模板语法.html，其余 40 个一次改完。
...
继续批量升级 apps/vue3/，从 `[上次最后一个文件路径]` 的下一个文件开始。
标准不变：Vue 3 官方文档权威、§零 工作流、四区块对齐、SCRIPT 分段+中文注释、面试题可验证。
...
1. 从 `01-基础语法/02-条件列表与key.html` 开始（**跳过**已完成的 `01-应用实例与模板语法.html`）
2. 按 §十二 清单顺序逐个修改，**不要**只分析不写文件
3. **默认目标：一次对话改完剩余 40 个文件**；上下文不够则至少完成 8～10 个，再用 §十 续跑
...
- `apps/vue3/src/01-基础语法/01-应用实例与模板语法.html`（**已完成**，跳过或仅做一致性检查）
...
- `apps/vue3/src/02-响应式与副作用/01-ref-reactive与解构.html`
- `apps/vue3/src/02-响应式与副作用/02-computed-watch-watchEffect.html`
- `apps/vue3/src/02-响应式与副作用/03-toRef-toRefs.html`
- `apps/vue3/src/02-响应式与副作用/04-readonly-shallow-markRaw.html`
- `apps/vue3/src/02-响应式与副作用/05-effectScope与副作用清理.html`
- `apps/vue3/src/02-响应式与副作用/06-onWatcherCleanup与watch清理.html`
- `apps/vue3/src/02-响应式与副作用/07-ref-reactive-shallowRef对比.html`

### 03-生命周期与组合式（4）

- `apps/vue3/src/03-生命周期与组合式/01-生命周期钩子.html`
- `apps/vue3/src/03-生命周期与组合式/02-composable复用.html`
- `apps/vue3/src/03-生命周期与组合式/03-setup与script-setup.html`
- `apps/vue3/src/03-生命周期与组合式/04-全量钩子与KeepAlive.html`

### 04-组件通信（6）

- `apps/vue3/src/04-组件通信/01-props与emit.html`
- `apps/vue3/src/04-组件通信/02-attrs与透传.html`
- `apps/vue3/src/04-组件通信/03-provide-inject.html`
- `apps/vue3/src/04-组件通信/04-组件v-model.html`
- `apps/vue3/src/04-组件通信/05-defineModel组件v-model.html`
- `apps/vue3/src/04-组件通信/06-props解构保持响应式.html`

### 05-插槽与组件形态（2）

- `apps/vue3/src/05-插槽与组件形态/01-默认具名作用域插槽.html`
- `apps/vue3/src/05-插槽与组件形态/02-动态组件与异步组件.html`

### 06-内置组件（4）

- `apps/vue3/src/06-内置组件/01-KeepAlive.html`
- `apps/vue3/src/06-内置组件/02-Teleport.html`
- `apps/vue3/src/06-内置组件/03-Suspense.html`
- `apps/vue3/src/06-内置组件/04-Transition.html`
